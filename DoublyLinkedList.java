public class DoublyLinkedList<T>
{
    ...
    public void insertBehindSelected(InsertionStrategy<T> s){
      Element current = first; // erstes object
      while(current != null){
        Element candidate = current; // candidate = first
        current = current.getSucc();
        if(s.select(candidate.getContent())){
          candidate.connectAsSucc(new Element(s.insert(candidate.getContent())));
          if(current != null){
            candidate.getSucc().connectAsSucc(current);
          } else{
            last = candidate.getSucc();
          }
          size++;
        }
      }
